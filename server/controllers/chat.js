...
const getMyChat = tryCatch(async (req, res, next) => {
    ...
    const transformedChats = chats.map(({ _id, name, groupChat, members }) => {
        ...
        return {
            _id,
            groupChat, avatar: groupChat ? members.slice(0, 3).map(({ avatar }) => avatar.url) : [otherMember.avatar.url],
...
const removeMembers = tryCatch(async (req, res, next) => {
    ...
    emitEvent(
        ...
        ALERT,
        chat.members,
        { message: `${userThatWillBeRemoved.name} has been removed from the froup`, chatId }
    );
...
const sendAttachments = tryCatch(async (req, res, next) => {
    ...
    if (files.length > 5) return next(new ErrorHandler("Files cannot be more then 5", 400));
...
const deleteChat = tryCatch(async (req, res, next) => {
    ...
    //// Here we delete all the messages and related attachments from cloudinary...
...
const getMessages = tryCatch(async (req, res, next) => {
    ...
    const [messages, totalMessagesCount] = await Promise.all([
        Message.find({ chat: chatId }).sort({ createdAt: -1 }).skip(skip).limit(resultPerpage).populate("sender", "name").lean(), Message.countDocuments({ chat: chatId }),
    ]);
